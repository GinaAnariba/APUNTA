...
var game = { //obejto:entidad con propiedades
  ...
  // Código para dibujar el menú de bienvenida
  function drawWelcomeMenu() {
    ctx.fillStyle = "#000"; // Color de fondo
    ctx.fillRect(0, 0, can.width, can.height);
  
    ctx.fillStyle = "#FFF"; // Color del texto
    ctx.font = "40px Times New Roman"; // Fuente del texto
    ctx.textAlign = "center";
  
    // Dibuja el título del juego en el centro del lienzo
    ctx.fillText("¡Bienvenido al Juego de la Navecita! ¡APUNTA!", can.width / 2, can.height / 2 - 50);
  
    // Dibuja el botón de inicio como un rectángulo con texto dentro
    var buttonWidth = 200;
    var buttonHeight = 50;
    var buttonX = can.width / 2 - buttonWidth / 2;
    var buttonY = can.height / 2 + 50;
  
    ctx.fillStyle = "#F00"; // Color del botón
    ctx.fillRect(buttonX, buttonY, buttonWidth, buttonHeight);
  
    ctx.fillStyle = "#FFF"; // Color del texto del botón
    ctx.font = "20px Times New Roman"; // Fuente del texto del botón
    ctx.fillText("Iniciar", can.width / 2, buttonY + buttonHeight / 2);
  
    // Agrega un controlador de eventos para el clic en el botón de inicio
    can.addEventListener("click", handleStartButtonClick);
  }
  ...
  class Player{
    ...
    update(){
      ...
      if (key[39] || key[68]) {
        ...
        if (this.position.x + this.velocity.x <= can.width) {
          this.position.x += this.velocity.x;
        }
      }
      if (key[40] || key[83]) {
        // Mover hacia abajo
        if (this.position.y + this.velocity.y <= can.height) {
          this.position.y += this.velocity.y;
        }
      }
    }
  ...
  function draw(){
    ...
    if(game.lives < 0) {
      ...
      // Espera 3 segundos antes de volver a mostrar el menú inicial
      setTimeout(function() {
        // Limpia el lienzo
        clean();
        // Llama a la función drawWelcomeMenu para mostrar el menú de bienvenida
        drawWelcomeMenu();
      }, 3000);
  ...
  function handleStartButtonClick(event) {
    ...
    var mouseX = event.clientX - can.getBoundingClientRect().left;
    var mouseY = event.clientY - can.getBoundingClientRect().top;
    ...
    if (
      ...
      // Elimina el controlador de eventos del clic en el botón de inicio
      can.removeEventListener("click", handleStartButtonClick);
  
      // Limpia el lienzo antes de iniciar el juego
      clean();
  
      // Llama a la función setup para inicializar el juego
      setup();
  
      // Inicia el bucle principal del juego llamando a la función draw
